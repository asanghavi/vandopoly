...
public class NetworkedNotificationManager {
	// Used for event subscribing
	// EventCallback holds the object to be updated and the method to be called
	private HashMap<String, ArrayList<NetworkedEventCallback>> listsOfEventObservers_ = null;
	private static NetworkedNotificationManager INSTANCE = null;
	
	private NetworkedNotificationManager() {
		listsOfEventObservers_ = new HashMap<String, ArrayList<NetworkedEventCallback>>();
	}
	
	public static NetworkedNotificationManager getInstance() {
		if (INSTANCE == null)
			INSTANCE = new NetworkedNotificationManager();
		
		return INSTANCE;		
	}
	
	private void addObserver(String event, Object subscriber, Method callBackMethod, boolean sendEvent) {
		//Check if an ArrayList doesn't exist yet for key value
		if (!listsOfEventObservers_.containsKey(event))
			listsOfEventObservers_.put(event, new ArrayList<NetworkedEventCallback>());
		
		NetworkedEventCallback observer;
		if(sendEvent)
			observer = new NetworkedEventCallback(subscriber, callBackMethod, event);
		else
			observer = new NetworkedEventCallback(subscriber, callBackMethod, "");
		
		ArrayList<NetworkedEventCallback> list = listsOfEventObservers_.get(event);
	...
	public void addObserver(String event, Object subscriber, String callbackMethodName, boolean sendEvent) {
		...
		addObserver(event, subscriber, callbackMethod, sendEvent);
	}
	
	// Completely removes observer object from all event Observer lists
	public void removeEventObserver(Object obj) {
		Collection<ArrayList<NetworkedEventCallback>> unionObservers = listsOfEventObservers_.values();
		
		for (ArrayList<NetworkedEventCallback> observerList : unionObservers) {
			
			//Iterate from high to low because ArrayList shifts everything down
			for (int i = observerList.size() - 1; i >= 0; i--) {
				NetworkedEventCallback observer = observerList.get(i);
	...
	public void removeEventObserver(String event, Object obj) {
		ArrayList<NetworkedEventCallback> observerList = listsOfEventObservers_.get(event);
